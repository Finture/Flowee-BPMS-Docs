...
*`$DISTRIBUTION_PATH` represents the path of the downloaded pre-packaged Flowee BPMS distribution, e.g., `camunda-bpm-tomcat-$PLATFORM_VERSION.zip` or `camunda-bpm-tomcat-$PLATFORM_VERSION.tar.gz` for Tomcat etc.
*`$PLATFORM_VERSION` denotes the version of Flowee BPMS you want to install, e.g., `7.1.0`.
...
For migration from **Flowee BPMS.0** to **Flowee BPMS.1**, the provided update scripts that match your database have to be executed.
...
To migrate your process application from Flowee BPMS.0 to Flowee BPMS.1, you need to follow these steps:
...
Flowee BPMS.1 ships with a new version of the JBoss AS 7, namely, 7.2.0.
