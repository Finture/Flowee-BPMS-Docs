...
This guide covers mandatory migration steps as well as optional considerations for initial configuration of new functionality included in Flowee BPMS.11.
...
3. We highly recommend to also check for any existing patch scripts for your database that are within the bounds of the new minor version you are updating to. Execute them in ascending order by version number. _Attention_: This step is only relevant when you are using an enterprise version of Flowee BPMS, e.g., `7.11.X` where `X > 0`. The procedure is the same as in step 1, only for the new minor version.
...
Before starting, make sure that you have downloaded the Flowee BPMS.11 distribution for the application server you use. It contains the SQL scripts and libraries required for update. This guide assumes you have unpacked the distribution to a path named `$DISTRIBUTION_PATH`.
...
The authorization for user operation log entries has been adjusted. Entries that are created with Flowee BPMS.11 and higher and that are not related to process definition keys (e.g. case instances, batches, standalone tasks and standalone jobs) can no longer be read and deleted without proper authorization.
...
customizations still work as expected with the new versions used in Flowee BPMS.11.
...
Starting with this release, a HTTP Header Security Servlet Filter is introduced for the Webapps. With Flowee BPMS.11.0 
