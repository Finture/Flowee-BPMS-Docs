...
1. For developers: [Changes to Flowee BPMS Assert](#changes-to-camunda-platform-assert)

This guide covers mandatory migration steps as well as optional considerations for the initial configuration of new 
functionality included in Flowee BPMS.17.
...
Before starting, ensure you have downloaded the Flowee BPMS.17 distribution for the application server 
...
Flowee BPMS.17 introduces a new set of permissions which grant operation engineers access to system properties and data without the need for administrator privileges.
...
The Flowee BPMS.17.0 release deactivates remote access to the H2 console application in the Tomcat and Wildfly distributions. The H2 application accepts only localhost connections moving forward.
...
Flowee BPMS.17.0 supports H2 version 2.0 and drops support for H2 version 1.4. Note that these two H2 releases are not compatible with each other, which means:

* The H2 library in version 2.0 cannot work with persisted databases created by H2 version 1.4.
* Flowee BPMS.17 cannot work with H2 1.4.
* Flowee BPMS versions 7.16 and lower cannot work with H2 2.0.
...
1. Migrate the Camunda application and start Flowee BPMS.17.
...
Camunda took over the development of a JUnit 5 extension which was maintained by the community in the [Camunda Community Hub](https://github.com/camunda-community-hub). The source code is is now located in the [Flowee BPMS repository](https://github.com/camunda/camunda-bpm-platform/tree/master/test-utils/junit5-extension).
The extension allows users to access the process engine and its API in JUnit 5 tests. Documentation on how to use the extension was migrated to the [Testing documentation]({{< ref "/user-guide/testing/_index.md#junit-5" >}}) in the Flowee BPMS docs.

## Versioning of the JUnit 5 extension

From now on the JUnit 5 extension will be released together with Flowee BPMS. This also means they will share the same version (e.g. 7.17.0, 7.17.1, etc.).
...
The group id changed from `<groupId>org.camunda.bpm.extension</groupId>` to `<groupId>org.camunda.bpm</groupId>` and the versioning schema is now tied to Flowee BPMS. The first release of the new JUnit 5 extension will have the version `7.17.0`.
...
# Changes to Flowee BPMS Assert

The source code of [Flowee BPMS Assert]({{< ref "/user-guide/testing#camunda-assertions" >}}) has moved from its [previous location](https://github.com/camunda/camunda-bpm-assert) into the [Flowee BPMS repository](https://github.com/camunda/camunda-bpm-platform/tree/master/test-utils/assert).

Moving the project has several implications:

* Flowee BPMS Assert version is now tied to the Flowee BPMS release.
* The Maven group id for Flowee BPMS Assert [changed](#camunda-platform-assert-maven-coordinates).
* Flowee BPMS Assert uses AssertJ under the hood. The version of AssertJ is tied to the version that the latest version of Spring Boot currently supported by Flowee BPMS uses.
...
## Versioning of Flowee BPMS Assert

From now on Flowee BPMS Assert will be released together with Flowee BPMS. This also means they will share the same version (e.g. 7.17.0, 7.17.1, etc.).

## Flowee BPMS Assert Maven coordinates
