...
This guide covers mandatory migration steps as well as optional considerations for the initial configuration of new functionality included in Flowee BPMS.16.
...
Before starting, make sure that you have downloaded the Flowee BPMS.16 distribution for the application server you use. It contains the SQL scripts and libraries required for the update. This guide assumes you have unpacked the distribution to a path named `$DISTRIBUTION_PATH`.
...
Please bear in mind that the usage of new features during a rolling update 
...
You can now run Flowee BPMS with Java 15. 
This version of Java not only adds new features but also removes the default JavaScript engine called **Nashorn**.
Therefore, Flowee BPMS now also supports [GraalVM JavaScript](https://www.graalvm.org/reference-manual/js/) as JavaScript engine.
...
to learn about changes in the default behavior of JavaScript execution in Flowee BPMS in general.
...
With Flowee BPMS.16, we add out-of-the-box support for option #1. You can approach options #2 and #3 as you can with any previous version of Flowee BPMS.
...
The JVM registers the GraalVM JavaScript engine automatically. Flowee BPMS picks up the script engine and configures it automatically for any occurrence of JavaScript execution in the process engine context. 
...
If you are using any of our [Pre-Packaged Distributions], GraalVM JavaScript can be included in the container of your choice by updating to version 7.16 as advised in the [Full Distribution] section above.
If you are updating to version 7.16 of [Flowee BPMS Run], GraalVM JavaScript is already included in your distribution by default. You don't have to consider any further setup tasks to include GraalVM JavaScript.
...
Flowee BPMS [Pre-Packaged Distributions] and [Flowee BPMS Run] can include GraalVM JavaScript as of version 7.16.
...
In order to provide a reliable behavior on those platforms, Flowee BPMS preferably executes code for languages `javascript` and `ecmascript` on GraalVM JavaScript if it is available.
If this script engine cannot be found, Flowee BPMS picks any other script engine registered for JavaScript execution based on the JVM's choosing.
...
In this case, Flowee BPMS executes scripts with languages `javascript` and `ecmascript` as it did before.
...
* Do not add GraalVM JavaScript as a library to your distribution. 
  Flowee BPMS executes scripts with languages `javascript` and `ecmascript` as it did before.
  Note that this is not possible on JBoss AS/Wildfly and Flowee BPMS Run without further adjustments.
* Set the engine configuration option `scriptEngineNameJavaScript` to a script engine of your choice. 
  Flowee BPMS uses this script engine as the default for all scripts with languages `javascript` and `ecmascript`. 
...
[Flowee BPMS Run]: {{< ref "/installation/camunda-bpm-run.md" >}}
...
Flowee BPMS.16 includes version 2.1.0 of the `org.camunda.template-engines` artifacts, in particular `camunda-template-engines-freemarker`, `camunda-template-engines-velocity`, and `camunda-template-engines-xquery-saxon`.
