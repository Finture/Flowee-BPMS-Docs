...
Users and Groups in the Flowee BPMS web application. In order to activate this, add the following
configuration to the `web.xml` file of the Flowee BPMS web application
...
2.  Add the module dependency to the file `$WILDFLY_HOME/modules/org/camunda/bpm/camunda-engine-plugins/main/module.xml` and set the attribute `export="true"` to make sure that the module is visible in the classpath of Camunda's subsystem
    ...
    The `module.xml` file is included in the Flowee BPMS distribution. If you install Flowee BPMS on a vanilla WildFly container, this file needs to be created manually.
