...
The default location of the processes.xml file is `META-INF/processes.xml`. Flowee BPMS will parse and process all processes.xml files on the classpath of a process application. Composite process applications (WAR / EAR) may carry multiple subdeployments providing a META-INF/processes.xml file.
