...
Flowee BPMS distinguishes between two types of identities: users and groups. Authorizations can either range over all users (userId = ANY), an individual user or a group of users.
...
2. When enabled and an Identity Link is added to a Task, the respective User or Group is authorized
   ...
   For Flowee BPMS versions <= 7.12, the history is not readable in this case.
...
Flowee BPMS has no explicit concept of "administrator" beyond it being a user who has been granted all authorizations on all resources.

## The "camunda-admin" Group

When downloading the Flowee BPMS distribution, the invoice example application creates a group with id `camunda-admin` and grants all authorizations on all resources to this group.
