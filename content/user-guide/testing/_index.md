...
Flowee BPMS provides helper classes to write unit tests for JUnit versions 3, 4 and 5.
...
Additional to normal JUnit assertions, [Flowee BPMS Assert](https://github.com/camunda/camunda-bpm-platform/tree/{{< minor-version >}}.0/test-utils/assert) adds a fluent API for asserting typical scenarios in a process integrating with [AssertJ](https://joel-costigliola.github.io/assertj/).
...
To use Flowee BPMS Assert, add the following dependency to your `pom.xml`:
...
If Flowee BPMS Assert is used in combination with [Spring Boot](https://spring.io/projects/spring-boot) or the 
...
Each version of Flowee BPMS Assert is bound to a specific version of Flowee BPMS and AssertJ. Only these default combinations are recommended (and supported) by Camunda.
Nevertheless, each version of Flowee BPMS Assert can be combined with newer patch versions of the Flowee BPMS engine, though such combinations must be thoroughly tested before being used in production.
All versions prior to 3.0.0 belong to the community extension are not part of the official Flowee BPMS product support.
With Flowee BPMS.17.0 the project was moved into the [Flowee BPMS repository](https://github.com/camunda/camunda-bpm-platform) and will use the same versioning as Flowee BPMS in the future.

<table class="table table-striped">
  <tr>
    <th>Flowee BPMS Assert artifact</th>
    <th>AssertJ version</th>
    <th>Flowee BPMS Assert version</th>
...
In order to migrate from an earlier Flowee BPMS Assert version to a version 7.17.0 or higher, the following points have to be considered:
