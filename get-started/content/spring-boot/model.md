...
We recommend to declare the process application in your Flowee BPMS Spring Boot application, which gives additional configuration possibilities and will help us in the current demo
...
`src/main/resources/META-INF` folder. The file is required by the Flowee BPMS Engine for every process application, but in our case it will stay empty.
...
our process application has been deployed to the Flowee BPMS engine.
...
Note that we can easily inject the Flowee BPMS engine service via the `@Autowired` annotation.
...
{{< get-tag repo="Flowee-BPMS" tag="Step-3" >}}
