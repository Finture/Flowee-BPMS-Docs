...
2. Add the Flowee BPMS & Spring Boot dependencies
...
## Add Flowee BPMS Platform & Spring Boot Dependencies
...
We add the Spring Boot BOM in the "dependency management" section and the Flowee BPMS Spring Boot Starter for Webapps, which will automatically include the Flowee BPMS engine and webapps in the app.
...
<project xmlns="http://maven.apache.org/POM/4.0.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://maven.apache.org/POM/4.0.0 http://maven.apache.org/xsd/maven-4.0.0.xsd">
  ...
  <groupId>org.finture.bpm.getstarted</groupId>
  <artifactId>loan-approval-spring-boot</artifactId>
  <version>0.0.1-SNAPSHOT</version>

  <properties>
    <flowee.spring-boot.version>0.7.1</flowee.spring-boot.version>
  ...
  <dependencies>
    <dependency>
      <groupId>org.finture.bpm.springboot</groupId>
      <artifactId>flowee-bpm-spring-boot-starter-webapp</artifactId>
      <version>${flowee.spring-boot.version}</version>
    </dependency>
    <dependency>
...
The class is added in the `src/main/java` folder in the `org.finture.bpm.getstarted.loanapproval` package.

```java
package org.finture.bpm.getstarted.loanapproval;
...
Our first Flowee BPMS Spring Boot application is ready now. As a result of the build, you will have a JAR-file in your `target` folder. This JAR is a Spring Boot application,
which embeds inside Tomcat as a web container, Flowee BPMS engine and Flowee BPMS Web applications resources.
When started, it will use an in-memory H2 database for Flowee BPMS Engine needs.
...
```
Then go to [http://localhost:8080/](http://localhost:8080/) in your browser and enjoy the Flowee BPMS webapps.

Another way to run the app is to simply run the JAR-file with a `java -jar` command.

{{< get-tag repo="FLowee-BPMS" tag="Step-1" >}}
